...
pub struct AutomaticCheckpointConfig {
    ...
    /// Maximum automatic checkpoints retained for one non-zero session.
    /// Zero disables the per-session checkpoint cap.
    ...
    fn choose_target(
        ...
        if request.result_state == SlotState::Anchor
            && (anchor_count >= self.config.max_anchors as usize
                || session_anchor_count >= self.session_anchor_limit(request))
        {
            return self
                .slots
                .iter()
